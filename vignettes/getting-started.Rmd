...
object at the point it is consumed by `sdr()`, `itmle()`, or
`sdr_competing()` / `itmle_competing()`. The trim quantile is computed over
all time points present in the weight object, not just those within the
intervention window or the current `tmax`. This means:
...
- The same `trim` value must be passed consistently to all estimators
  (main and competing-event) that share a weight object.
...
## Step 6 — Competing-event (in-hospital mortality) variant

`sdr_competing()` and `itmle_competing()` estimate in-state survival (e.g.
in-hospital mortality) under the same MTP. They are included specifically to
allow reuse of **exactly the same weight object** — with the same trim — as
the main outcome estimators. This matters when the MTP has a limited
intervention duration: both the outcome and the competing-event analysis must
operate on identically trimmed weights for the two results to be
directly comparable.

The estimators are based on the competing-event / survival LMTP framework of
Díaz, Hoffman & Hejazi (2023). The only reason a separate implementation
exists within this package (rather than using `lmtp` directly) is to share
the pre-computed, trimmed density-ratio weights from `density_ratio()`.

One practical detail: the competing-event estimators may require additional
time points beyond the main `tmax` to cover the full at-risk period.
Unit weights ($r_t = 1$) for these extra time points are appended to the
weight object **after** the global trim has been applied, so trimming remains
consistent with the main analysis.

```{r competing}
res_mort <- sdr_competing(
  DT              = df,
  ...
  sl_q            = sl_lib,
  inner_v         = 5L,
  trim            = 0.99,   # must match trim used in sdr() / itmle()
  parallel        = FALSE
)

cat(sprintf("SDR competing  psi = %.3f  SE = %.3f\n",
            res_mort$psi, res_mort$se))
```
...
Díaz I, Hoffman KL, Hejazi NS (2023). Causal Survival Analysis under
