...
itmle_eic_se <- function(ic, cluster_by_id = NULL, use_second_moment = TRUE) {
  ...
  ic_c <- dt$ic - mean(dt$ic)
  dt[, ic_c := ic_c]
  S <- dt[, .(S = sum(ic_c), n = .N), by = cl]
  G <- nrow(S)
  ...
  se <- sqrt((G / (G - 1)) * sum(S$S^2) / (n_eff^2))
...
itmle <- function(
  ...
  fold_worker <- function(f_idx) {
    ...
    for (tt in rev(seq_len(tmax))) {
      ...
      p_rem_shf_vl       <- numeric(0)
      p_dex_nat_vl       <- numeric(0)
      p_dex_shf_vl       <- numeric(0)
      q_death_nat_vl_raw <- numeric(0)
      q_dc_nat_vl_raw    <- numeric(0)
      q_death_shf_vl_raw <- numeric(0)
      q_dc_shf_vl_raw    <- numeric(0)
      q_rem_nat_vl       <- numeric(0)
      q_rem_shf_vl       <- numeric(0)
