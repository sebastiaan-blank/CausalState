...
print.branch_cal_summary <- function(x, digits = 3L, ...) {
  ...
  # column widths: n_vl is integer, rest are rounded floats
  w_n  <- 6L
  w_f  <- digits + 4L   # e.g. digits=3 -> "0.123" width 5, plus sign/space -> 6
  col_widths <- c(w_n, w_f, w_f, w_f)
  branch_sep <- " | "
  sep_w      <- nchar(branch_sep)

  branch_w <- sum(col_widths) + (length(col_widths) - 1L)  # spaces between cols
  ...
  # header line 1: branch names spanning their columns
